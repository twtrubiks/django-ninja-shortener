...
def test_home_view_with_session_data(client):
    """測試 home_view 在 session 中有 latest_short_url 時的行為。"""
...
def test_dashboard_view_authenticated(authenticated_client, authenticated_user):
    ...
    Link.objects.create(original_url="http://myurl.com", short_code="mycode", owner=authenticated_user)
    Link.objects.create(original_url="http://otherurl.com", short_code="othercode", owner=other_user)

    response = authenticated_client.get(reverse('dashboard'))
    assert response.status_code == 200
    assert 'dashboard.html' in [t.name for t in response.templates]
    # 檢查 context 中的 links 數量是否正確
...
def test_shorten_url_get_request(client):
    """測試 GET 請求 shorten_url_view 會被重定向。"""
...
def test_register_view_post_success(client):
    ...
    # A successful registration should redirect to the login page.
    assert response.status_code == 302
